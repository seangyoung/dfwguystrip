...
import { activities, candidateDates, itineraries, type Availability } from './types'
import { hasSupabase, isCoordinator, supabase } from './supabase'

type View = 'home' | 'activities' | 'itineraries' | 'vote' | 'calendar' | 'admin'
...
function Activities() { const [filter, setFilter] = useState<'all' | 'flagship' | 'alternate'>('all'); const visible = activities.filter(a => filter === 'all' || a.tier === filter)
  return <section className="section page"><p className="eyebrow">ACTIVITY CATALOG</p><h1>The possible stories.</h1><p className="lede">Cost figures marked “Estimate” are working planning numbers. Follow each source link for current rules and prices before booking.</p><div className="filters">{(['all', 'flagship', 'alternate'] as const).map(f => <button key={f} className={filter === f ? 'selected' : ''} onClick={() => setFilter(f)}>{f === 'all' ? 'Everything' : f}</button>)}</div><div className="activity-grid">{visible.map(a => <article className="activity" key={a.id}><div className="activity-top"><span>{a.category}</span><b className={a.tier}>{a.tier}</b></div><h2>{a.name}</h2><strong>{a.estimate}</strong><p>{a.note}</p><dl><div><dt>Requirements</dt><dd>{a.restriction}</dd></div><div><dt>Timing</dt><dd>{a.seasonal}</dd></div></dl><a href={a.sourceUrl} target="_blank" rel="noreferrer">Official source ↗</a></article>)}</div></section> }
...
function Vote({ user }: { user: SessionUser }) { const [ranked, setRanked] = useState(['zero-g', 'stockyards', 'gun-experience']); const [avoid, setAvoid] = useState<string[]>([]); const [comment, setComment] = useState(''); const [saved, setSaved] = useState(false); const [saveError, setSaveError] = useState('')
  ...
  const available = activities.filter(a => !ranked.includes(a.id))
  ...
  return <section className="section page ballot"><p className="eyebrow">PREFERENCE BALLOT</p><h1>What would make it great?</h1><p className="lede">Rank your top three. Then mark any activity you would rather sit out. There is no wrong answer here.</p><div className="ballot-grid"><div><h2>Your top three</h2>{ranked.map((id, index) => { const a = activities.find(activity => activity.id === id)!; return <div className="rank" key={id}><b>{index + 1}</b><span>{a.name}</span><button disabled={index === 0} onClick={() => move(index, -1)}>↑</button><button disabled={index === ranked.length - 1} onClick={() => move(index, 1)}>↓</button></div> })}<select defaultValue="" onChange={event => { if (event.target.value && ranked.length < 3) setRanked([...ranked, event.target.value]); event.currentTarget.value = '' }}><option value="">Add an activity</option>{available.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}</select></div><div><h2>Would rather not</h2>{activities.map(a => <label className="check" key={a.id}><input type="checkbox" checked={avoid.includes(a.id)} onChange={() => setAvoid(avoid.includes(a.id) ? avoid.filter(id => id !== a.id) : [...avoid, a.id])} />{a.name}</label>)}</div></div><label className="comment-label">Anything we should know?<textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Budget, mobility, an idea we missed, or anything else." /></label><button className="primary" onClick={saveBallot}>Save my ballot</button>{saved && <p className="saved">{supabase ? 'Your ballot is saved.' : 'Saved locally for this preview.'}</p>}{saveError && <p className="error">Could not save: {saveError}</p>}</section> }
